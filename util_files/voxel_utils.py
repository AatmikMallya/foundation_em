...
def get_subvols_batched(init_boxes_zyx, datatype):
    ...
    dataset_3d = ts.open({
        ...
        'kvstore': kvstore,
        'context': {'cache_pool': {'total_bytes_limit': 300_000_000_000}},
        'recheck_cached_data': 'open'
...
def get_full_mito_coords_batched(mito_df, batch_size=1000):
    """
    Get full coordinates for multiple mitochondria in batches.
    
    Parameters:
    -----------
    mito_df : pandas.DataFrame
        DataFrame containing mitochondria information with columns:
        - bodyId: neuron ID
        - x, y, z: center coordinates
        - size: volume in voxels
    batch_size : int
        Number of mitochondria to process in each batch
    
    Returns:
    --------
    dict
        Dictionary mapping mitochondrion indices to arrays of coordinates
    """
    results = {}
    
    # Process in batches
    for batch_start in range(0, len(mito_df), batch_size):
        batch_end = min(batch_start + batch_size, len(mito_df))
        batch = mito_df.iloc[batch_start:batch_end]
        
        # print(f"Processing batch {batch_start//batch_size + 1}: mitochondria {batch_start} to {batch_end-1}")
        
        # First, adjust centers for all mitochondria in batch
        adjusted_centers = []
        for i, mito in batch.iterrows():
            bodyId = mito['bodyId']
            center_xyz = mito[['x', 'y', 'z']].to_numpy()
            center = adjust_center(center_xyz, 1, bodyId, data_instance='mito-objects')
            if center is None:
                results[i] = np.array([], dtype=int).reshape(0, 3)
                continue
            adjusted_centers.append((i, center, bodyId))
        
        # If all centers were invalid, continue to next batch
        if not adjusted_centers:
            continue
        
        # Initialize data structures for each mitochondrion
        all_box_zyx = {i: [] for i, _, _ in adjusted_centers}
        mito_coords_zyx = {i: np.array([], dtype=int).reshape(3, 0) for i, _, _ in adjusted_centers}
        is_cropped = {i: np.array([True]) for i, _, _ in adjusted_centers}
        
        # Iteratively expand boxes until all mitochondria are fully captured
        while any(np.any(crop) for crop in is_cropped.values()):
            # Prepare boxes for this batch
            current_batch_boxes = []
            batch_indices = []
            batch_body_ids = []
            
            for i, center, bodyId in adjusted_centers:
                if not np.any(is_cropped[i]):
                    continue  # Skip if this mito is already fully processed
                
                # Calculate radius based on size or use current cropped areas
                if not all_box_zyx[i]:
                    # First iteration - use size-based estimate
                    mito_size = mito_df.loc[i, 'size']
                    dr = int(((3 * mito_size / (np.pi * 4)) ** (1/3)) * 1.5)
                    box_zyx = [np.flip(center) - dr, np.flip(center) + dr]
                else:
                    # Find a cropped coordinate to center next box on
                    cropped_coords_zyx = mito_coords_zyx[i][:, is_cropped[i]]
                    if cropped_coords_zyx.shape[1] == 0:
                        continue
                    random_idx = np.random.randint(cropped_coords_zyx.shape[1])
                    new_center = cropped_coords_zyx[:, random_idx]
                    dr = 100  # Larger expansion for subsequent iterations
                    box_zyx = [new_center - dr, new_center + dr]
                
                current_batch_boxes.append(box_zyx)
                batch_indices.append(i)
                batch_body_ids.append(bodyId)
            
            if not current_batch_boxes:
                break  # All mitochondria are fully processed
            
            # Process boxes one by one instead of using batch function
            # This avoids the inhomogeneous shape error
            for box_idx, (box_zyx, i, bodyId) in enumerate(zip(current_batch_boxes, batch_indices, batch_body_ids)):
                # Get mitochondrion subvolume
                subvol = get_subvol_any_size(box_zyx, 'mito-objects')
                
                # Get mitochondrion mask
                mito_subvol = get_center_object(subvol == bodyId)
                mito_idxs = np.array(np.where(mito_subvol))  # 3xn array of indices
                min_coord_zyx = np.array(box_zyx[0]).reshape((3, 1)).astype(int)
                
                # Track this box
                all_box_zyx[i].append(box_zyx)
                
                # Find new coordinates
                if mito_coords_zyx[i].shape[1] > 0:
                    # Check which coordinates are new
                    coord_not_in = []
                    for i_coord in [0, 1, 2]:
                        coord_not_in.append(~np.isin(box_zyx[0][i_coord] + mito_idxs[i_coord], mito_coords_zyx[i][i_coord]))
                    new_coord_bool = np.any(coord_not_in, axis=0)
                else:
                    new_coord_bool = np.ones((mito_idxs.shape[1],), dtype=bool)
                
                # Add new coordinates to the collection
                if np.any(new_coord_bool):
                    mito_coords_zyx[i] = np.unique(
                        np.concatenate((mito_coords_zyx[i], mito_idxs[:, new_coord_bool] + min_coord_zyx), axis=1), 
                        axis=1
                    )
                
                # Update cropped status for all coordinates
                if mito_coords_zyx[i].shape[1] > 0:
                    is_cropped[i] = np.ones((mito_coords_zyx[i].shape[1],), dtype=bool)
                    for this_box_zyx in all_box_zyx[i]:
                        min_zyx, max_zyx = this_box_zyx
                        good_bool = np.all([
                            np.all(mito_coords_zyx[i] > min_zyx[:, np.newaxis], axis=0),
                            np.all(mito_coords_zyx[i] < (max_zyx[:, np.newaxis] - 1), axis=0)
                        ], axis=0)
                        is_cropped[i][good_bool] = False
        
        # Convert to final xyz coordinates for each mitochondrion in the batch
        for i in batch.index:
            if i in mito_coords_zyx and mito_coords_zyx[i].shape[1] > 0:
                mito_coords_xyz = np.flip(mito_coords_zyx[i].T, axis=1)  # nx3 array of mito coordinates
                mito_coords_xyz[:, [0, 2]] = mito_coords_xyz[:, [2, 0]]
                results[i] = mito_coords_xyz.astype(int)
            else:
                results[i] = np.array([], dtype=int).reshape(0, 3)
