...
def get_AUC(X, Y, group_ids, k_groups, alpha, model = 'GLM', return_r2 = False):
    ...
    for k_group in k_groups:
        ...
        if return_r2:
            train_AUC += np.corrcoef( pred_train, Y_train )[0,1] / len(k_groups)
        else:
    ...
    if return_r2:
        ...
        test_AUC = np.corrcoef(pred_test, Y_test)[0,1]
    else:
        TP = np.zeros((num_points,)); FP = np.zeros((num_points,))
        for i, thresh in enumerate(np.linspace(0,1,num_points)):
...
def get_mito_pos_features():
    ...
    dist_bins = [presynapse_dist_bins, postsynapse_dist_bins, intra_neuron_mito_dist_bins]
    titles = ['Presynapse Histogram', 'Postsynapse Histogram', 'Intramitochondria Histogram']
...
def get_mito_position_space(neuron_types, arbors, offset = 0, jitter_strength = 0):
    ...
    all_Y = np.array( [ [] for _ in range(3) ] ).T
    mean_dx = np.zeros( 2 )
        ...
        for i_neuron in np.where(np.isin(neuron_quality_np[:,1], neuron_types))[0]:
            ...
            if neuron_quality[f'has_{arbor}'].iloc[i_neuron] and isfile(synapse_file):# and arbor == 'axon':
                ...
                if np.sum( bool_mito ) > 1 and len(arbor_synapse_df) > 1:
                    ...
                    #Y = np.append( np.zeros(shuffled_mito_df.shape[0]), np.ones(mito_df.shape[0]) ).astype(int)
...
def get_synapse_on_mito_features():

    # create feaures to use for GLM
    presynapse_dist_bins = np.array( [0,0.3,0.6,1.2,2.5] )
    postsynapse_dist_bins = np.array( [0,0.3,0.6,1.2,2.5] )

    dist_bins = [presynapse_dist_bins, postsynapse_dist_bins]
    titles = ['Presynapse Histogram', 'Postsynapse Histogram']

    scalar_features = []
    scalar_features.append( 'is in synapse cluster' )
    scalar_features.append( 'nearest neighboring synapse' )
    scalar_features.append( 'farthest neighboring synapse' )
    scalar_features.append( 'frac neighbors presynapse' )
    scalar_features.append( 'is presynapse' )
    scalar_features.append( 'branch order' )
    scalar_features.append( 'leaf number' )
    scalar_features.append( 'Daughter Circumference / Mother Circumference' )
    scalar_features.append( 'is on thicker daughter side' )
    scalar_features.append( 'distance up segment' )
    scalar_features.append( 'is on branch' )
    return dist_bins, titles, scalar_features

def get_synapse_on_mito_space(neuron_types, arbors, offset = 0):

    dist_bins, titles, scalar_features = get_synapse_on_mito_features()
    num_hist_bins = [ len(dist_bins[i_hist])-1 for i_hist in range(len(dist_bins)) ]
    num_features = len(scalar_features) + np.sum(num_hist_bins)
    ...
    for i_arbor, arbor in enumerate(arbors):
        for i_neuron in np.where(np.isin(neuron_quality_np[:,1], neuron_types))[0]:
            ...
            if neuron_quality[f'has_{arbor}'].iloc[i_neuron] and isfile(synapse_file):# and arbor == 'axon':
                ...
                if len(arbor_synapse_df) > 10:
                    X, Y = get_orphan_synapse_features(arbor_synapse_df, np.arange(len(arbor_synapse_df)), bodyId, arbor, offset = -np.inf)
                    all_X = np.append( all_X, X, axis=0 )
...
def get_orphan_synapse_features(synapse_df, i_synapses, bodyId, arbor, offset = -np.inf):
    ...
    is_pre = synapse_df['type'].to_numpy() == 'pre'
    rowIds = synapse_df['rowId'].to_numpy()
    links = synapse_df['link'].to_numpy()
    distances = synapse_df['distance'].to_numpy()
    neighbor_dists = [distances[np.any([node == rowIds, node == links],axis=0)] for node in rowIds ]
    min_neighbor_dists = np.array([ np.min(dists) for dists in neighbor_dists ])
    max_neighbor_dists = np.array([ np.max(dists[dists != np.inf]) for dists in neighbor_dists ])
    frac_neighbor_pre = np.array([np.mean(is_pre[np.any([node == rowIds, node == links],axis=0)]) for node in rowIds ])

    synapse_idxs = utils.find_closest_idxs(s_np, synapse_df)
    branch_orders = utils.get_branch_order( s_np[synapse_idxs,0], s_np )


    # compute feature space
    Y = [ len(eval(i_mitos_on)) == 0 for i_mitos_on in synapse_df['i_mitos_on'].to_numpy()[i_synapses] ] # True if this is an orphan synapse
    X = np.zeros( (len(Y),num_features) ) + offset

    syn_syn_dists = utils.get_pairwise_dists(synapse_df, s_np) * 8/1000
    for i_hist in [0,1]:
        # 'pre' then 'post'
        this_bins = dist_bins[i_hist]
        for i_d in range(len(this_bins)-1):
            bool_1 = syn_syn_dists[:,is_pre==(1-i_hist)] > this_bins[i_d]
            bool_2 = syn_syn_dists[:,is_pre==(1-i_hist)] <= this_bins[i_d+1]
            X[:,init_idx[i_hist] + i_d] = np.sum( np.all([bool_1,bool_2],axis=0) , axis=1)[i_synapses]
        assert init_idx[i_hist]+i_d+1 == final_idx[i_hist]
    X[:,final_idx[-1]+0] = ~utils.is_isolated_synapse(synapse_df)[i_synapses] # is in synapse cluster
    X[:,final_idx[-1]+1] = min_neighbor_dists[i_synapses] # nearest neighboring synapse distance
    X[:,final_idx[-1]+2] = max_neighbor_dists[i_synapses] # farthest neighboring synapse distance
    X[:,final_idx[-1]+3] = frac_neighbor_pre[i_synapses] # fraction of neighbors that are a presynapse
    X[:,final_idx[-1]+4] = is_pre[i_synapses] # is this a presynapse
    try:
        arbor_base_idx = utils.find_arbor_base_idx(s_np, node_classes == node_class_dict[arbor])
        X[:,final_idx[-1]+5] = branch_orders[i_synapses] - utils.get_branch_order( s_np[[arbor_base_idx],0], s_np )[0] # branch order of each synapse
    except:
        pass
    X[:,final_idx[-1]+6] = utils.get_leaf_number(s_np[synapse_idxs[i_synapses],0], s_np) # leaf number of each synapse

    synapse_coords = synapse_df[['x','y','z']].to_numpy()
    for ii, i_synapse in enumerate(i_synapses):
        d1_idxs, d2_idxs, m_idxs = get_d1_d2_m_seg_idxs(s_np[synapse_idxs[i_synapse],0], s_np, branch_nodes)
        if d2_idxs is not None:
            X[ii,final_idx[-1]+7] = np.mean(s_np[d1_idxs,10]) > np.mean(s_np[d2_idxs,10]) # is_on_thicker_daughter
        X[ii,final_idx[-1]+8] = np.log10( np.mean(s_np[d1_idxs,10]) / np.mean(s_np[m_idxs,10]) )

        theta, phi = s_np[synapse_idxs[i_synapse],[7,8]]
        xyz = np.array( utils.spherical_2_cart(1, theta, phi) )
        rem_down_idxs = utils.get_down_idxs(s_np, s_np[synapse_idxs[i_synapse],0], np.isin(s_np[:,0], branch_nodes), count_start=True)
        down_seg_dist = np.sum(s_np[rem_down_idxs[:-1],6]) - (np.sum(xyz*synapse_coords[i_synapse]) - np.sum(xyz*s_np[synapse_idxs[i_synapse],[1,2,3]]))
        X[ii,final_idx[-1]+9] = 1 - (down_seg_dist / np.sum( s_np[d1_idxs[d1_idxs!= rem_down_idxs[-1]],6] ))
    arbor_branch_nodes = np.array(branch_nodes)[ np.isin(branch_nodes, s_np[node_classes==node_class_dict[arbor],0]) ]
    arbor_branch_idxs = np.array([ np.where(s_np[:,0] == branch_node)[0][0] for branch_node in arbor_branch_nodes ])

    X[:,final_idx[-1]+10] = np.any( cdist( synapse_df[['x','y','z']].to_numpy()[i_synapses], s_np[arbor_branch_idxs,:][:,[1,2,3]] ) <= s_np[arbor_branch_idxs,4][np.newaxis,:], axis=1 )
